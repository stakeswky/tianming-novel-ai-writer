...
{
    public class ShuqutaParser : INovelParser
    {
        ...
        public NovelInfo? ParseNovelInfo(string html)
        {
            try
            {
                ...
                var title = doc.DocumentNode.SelectSingleNode("//div[@id='info']/h1")?.InnerText?.Trim()
            ...
            {
                TM.App.Log($"[ShuqutaParser] 解析小说信息失败: {ex.Message}");
                return null;
            }
        }
        ...
        public NovelCatalog? ParseCatalog(string html)
        {
            try
            {
                ...
                {
                    Title = doc.DocumentNode.SelectSingleNode("//div[@id='info']/h1")?.InnerText?.Trim()
                         ?? doc.DocumentNode.SelectSingleNode("//h1")?.InnerText?.Trim()
                         ?? string.Empty
                };
                ...
                foreach (var a in links)
                {
                    ...
                    if (!Regex.IsMatch(href, @"/book/\d+/\d+/\d+\.html", RegexOptions.IgnoreCase)) continue;
                    ...
                    if (Regex.IsMatch(title, @"^第[\d一二三四五六七八九十百千万零〇]+[章节回篇部集卷]\s*$"))
                    {
                        var titleAttr = a.GetAttributeValue("title", string.Empty).Trim();
                        if (!string.IsNullOrEmpty(titleAttr) && titleAttr.Length > title.Length)
                        {
                            title = HtmlEntity.DeEntitize(titleAttr);
                        }
                        else if (a.ParentNode != null)
                        {
                            var parentText = HtmlEntity.DeEntitize(a.ParentNode.InnerText ?? string.Empty).Trim();
                            if (parentText.Length > title.Length && parentText.Length < 200)
                            {
                                title = Regex.Replace(parentText, @"\s+", " ").Trim();
                            }
                        }
                    }
                    ...
                    if (!Uri.TryCreate(href, UriKind.Absolute, out _))
                    {
                        if (href.StartsWith("/"))
                        {
                            href = "http://www.shuquta.com" + href;
                        }
                        else
                        {
                            href = "http://www.shuquta.com/" + href;
            ...
            {
                TM.App.Log($"[ShuqutaParser] 解析目录失败: {ex.Message}");
                return null;
            }
        }
        ...
        public string ParseChapterContent(string html)
        {
            try
            {
                ...
                {
                    "底色", "字色", "字号", "滚屏",
                    "默认", "白色", "淡蓝", "淡灰", "灰色", "深灰", "暗灰", "绿色", "明黄", "黑色", "红色", "蓝色", "棕色",
                    "小号", "较小", "中号", "较大", "大号",
                    "最新网址", "www.shuquta.com", "shuquta.com", "说说520", "全文字更新", "牢记网址",
                    "上一页", "下一页", "加入书签", "目录", "投票推荐",
                ...
                foreach (var line in lines)
                {
                    ...
                    if (trimmed.Contains("正文") && trimmed.Contains("章") && trimmed.Length < 100) skip = true;
            ...
            {
                TM.App.Log($"[ShuqutaParser] 解析章节内容失败: {ex.Message}");
                return string.Empty;
            }
        }
