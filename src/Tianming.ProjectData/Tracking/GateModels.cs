    ...
    public class GateResult
    {
        ...
        private static string HumanizeError(FailureType type, string error)
        {
            if (type == FailureType.Protocol)
            {
                ...
                {
                    var m = Regex.Match(error, @"CHANGES协议违规：(.+?)必须为 ShortId");
                    var field = m.Success ? m.Groups[1].Value.Trim() : "实体ID字段";
                    var vm = Regex.Match(error, @"收到(?:非Id值|非法值)'([^']+)'");
            ...
            if (error.Contains("MovementStartLocationMismatch"))
            {
                ...
                return $"角色'{entityId}'的出发地点必须是账本当前位置。请修正CHANGES的FromLocation，或不要为该角色申报移动";
            }

            if (error.Contains("MovementChainBreak"))
            {
                var entityId = ExtractEntityId(error);
                return $"角色'{entityId}'本章多次移动时路径不连续：上一次ToLocation必须等于下一次FromLocation。请修正移动链或减少移动次数";
            }

            if (error.Contains("ItemOwnershipMismatch"))
            {
                var entityId = ExtractEntityId(error);
                return $"物品'{entityId}'的转让起点不符。请修正CHANGES的FromHolder，或不要申报该物品转让";
            }

            if (error.Contains("RelationshipContradiction"))
            {
                var entityId = ExtractEntityId(error);
                return $"角色'{entityId}'与他人关系申报矛盾：同一章内同一对角色不能同时声明盟友与仇敌。请统一关系结论并只保留一种";
            }

            if (error.Contains("LevelRegression"))
            {
                var entityId = ExtractEntityId(error);
                return $"角色'{entityId}'等级/阶段出现回退。除非正文明确发生失去/降级事件，否则不要让NewLevel低于账本记录";
            }

            if (error.Contains("AbilityLossWithoutEvent"))
            {
                var entityId = ExtractEntityId(error);
                return $"角色'{entityId}'声明失去能力但缺少原因。若LostAbilities非空，KeyEvent必须写清楚失去原因（封印/废除/代价等）";
            }

            if (error.Contains("TrustDeltaExceedsLimit"))
            {
                var entityId = ExtractEntityId(error);
                return $"角色'{entityId}'与他人的信任值变化幅度过大。请把TrustDelta控制在合理范围（默认±30以内），并用KeyEvent说明关系变化原因";
            }

            if (error.Contains("PayoffBeforeSetup"))
            {
                var entityId = ExtractEntityId(error);
                return $"伏笔'{entityId}'尚未埋设，不能在本章揭示。请先在正文中自然埋设该伏笔，或移除揭示动作";
            }

            if (error.Contains("ForeshadowingRollback"))
            {
                var entityId = ExtractEntityId(error);
                return $"伏笔'{entityId}'已揭示，不能重新埋设。请移除该伏笔的埋设动作";
            }

            if (error.Contains("ConflictStatusSkip"))
            {
                var entityId = ExtractEntityId(error);
                return $"冲突'{entityId}'的状态不能回退。请检查NewStatus是否正确，冲突只能向前推进";
            }

            if (error.Contains("CharacterNotInvolved"))
            {
                var entityId = ExtractEntityId(error);
                return $"角色'{entityId}'不在本章涉及角色列表中，但出现在CHANGES里。请让该角色在正文中登场，或从CHANGES中移除";
            }
        ...
        {
            var match = Regex.Match(error, @"实体:\s*([^,]+)");
            if (match.Success)
                return match.Groups[1].Value.Trim();

            match = Regex.Match(error, @"EntityId[:\s]+([^\s,\]]+)");
            if (match.Success)
                return match.Groups[1].Value;

            match = Regex.Match(error, @"角色\s+(\S+)\s+不在");
            if (match.Success)
                return match.Groups[1].Value;

            var colonIdx = error.IndexOf(':');
            if (colonIdx > 0 && colonIdx < error.Length - 1)
                return error[(colonIdx + 1)..].Trim().Split(new[] { ' ', ',' })[0];
