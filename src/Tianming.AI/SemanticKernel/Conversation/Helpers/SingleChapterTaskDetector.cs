...
namespace TM.Services.Framework.AI.SemanticKernel.Conversation.Helpers
{
    public static class SingleChapterTaskDetector
    {
        ...
        public static bool IsSingleChapterTask(string userInput)
        {
            ...
            var normalized = userInput.Replace(" ", string.Empty).ToLowerInvariant();
            ...
            foreach (var keyword in SingleKeywords)
            {
                if (normalized.Contains(keyword) && (normalized.Contains("章") || normalized.Contains("章节")))
                    return true;
            }
