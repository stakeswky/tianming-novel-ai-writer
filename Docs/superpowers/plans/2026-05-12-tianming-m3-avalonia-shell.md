...
public sealed class ThemeBridge
{
    ...
    private void ApplyCore(PortableThemeApplicationRequest request)
    {
        ...
        var variant = request.Plan.Theme == PortableThemeType.Dark
            ? ThemeVariant.Dark
            : ThemeVariant.Light;
        app.RequestedThemeVariant = variant;
        _currentVariant = request.Plan.Theme;
        _log.LogInformation("Applied theme {Theme}", request.Plan.Theme);
    }
...
Expected: 0 error。若报 `PortableThemeApplicationRequest.Plan` 无 `Theme` 属性，改为 `request.Plan.NextTheme`——以 `PortableThemeApplicationPlan` 的实际定义为准（读 `src/Tianming.Framework/Appearance/PortableThemeStateController.cs` 取确切属性名）。
