...
M3 完成形态由 Mac_UI 参考图定义：

| 模块 | 参考图 | 伪代码 |
|---|---|---|
| 主工作区三栏 | `Mac_UI/images/02-main-workspace-three-column.png` | `Mac_UI/pseudocode/02-main-workspace-three-column.md` |
| 欢迎 / 项目选择 | `Mac_UI/images/01-welcome-project-selector.png` | `Mac_UI/pseudocode/01-welcome-project-selector.md` |

视觉基建（chrome + status bar + design tokens + 14 primitives）见 spec `2026-05-13-tianming-mac-ui-foundation-design.md`；M3 视觉重做（让本 spec 设计对应的 view 跟参考图像素级对齐）见 sub-plan 2 实施。

后续如需修订视觉，**编辑 specs 分支上的 Mac_UI** 后单向同步到 main。
